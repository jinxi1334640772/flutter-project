...
class _FileUploadDownloadState extends State<FileUploadDownload> {
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          TextButton(
            onPressed: () async {
              String dirloc = '${await PathUtils.getDocumentsDirPath()}${Platform.pathSeparator}22.jpg';
              print("下载路径：$dirloc");
              XHttp.downloadFile("http://f.hiphotos.baidu.com/image/pic/item/4034970a304e251f503521f5a586c9177e3e53f9.jpg", dirloc);
            },
            child: const Text("下载"),
          )
        ],
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          TextButton(
            onPressed: () async {
              DownLoadFile.downloadFile("http://a.hiphotos.baidu.com/image/pic/item/8d5494eef01f3a292d2472199d25bc315d607c7c.jpg", "${Random().nextInt(2000)}.jpg", context);
            },
    ...
    return Container(
      ...
      child: Column(
        children: [
              ...
              TextButton(
                onPressed: () async {
                  FilePickerResult? result = await FilePicker.platform.pickFiles(
                    ...
                    allowedExtensions: ['jpg', 'png', 'pdf', 'doc', 'docx', 'xls', 'txt', 'xlsx', 'zip', 'rar'],
                  );
                  if(result != null){
                    print("单文件上传的文件信息：$result.files.single");
                    setState(() {
                      singleFileMap.putIfAbsent(result.files.single.name, () => result.files.single.path);
                    });
                  }
                },
          ...
          Column(
            children: fileWidget(singleFileMap)
          )
        ],
      )
    );
    ...
    return Container(
        ...
        child: Column(
          children: [
                  ...
                  onPressed: () async {
                    ...
                    if(result != null){
                      ...
                      for (var file in result.files) {
                        setState(() {
                          multipleFileMap.putIfAbsent(file.name, () => file.path);
                        });
                      }
                    }
                  },
            ...
            Column(
                children: fileWidget(multipleFileMap)
            )
          ],
        )
    );
          ...
          children: [
            Row(
              ...
              children: [
                ...
                TextButton(onPressed: _addCircleGeoFence, child: const Text("添加圆形围栏 100m"),),
  ...
  _requestPermission() async {
    ...
    if (!hasLocationPermission) {
      ...
      ToastUtils.showDialogPopup(context,  "您已拒绝位置定位权限, 请在设置中心同意app的权限请求. ", leftBtnText: "退出", rightBtnText: "去设置中心", successCallback: () async{
        /// 去设置中心操作
        /// 去设置中心  也是使用到了权限管理的包
  ...
  _startLocation() async {
    ///开始定位之前设置定位参数
      ...
      AmapLocation.instance
          .listenLocation(mode: LocationAccuracy.High, needAddress: true, interval: 2000)
  ...
  _addCircleGeoFence({double radius = 100, double longitude = 116.473115, double latitude = 39.993207}) async {
    /// 获取当前位置
    await _requestPermission();
    // 可通过状态判断是否在范围之内
    AmapLocation.instance.addCircleGeoFence(
      ...
      print('状态: ${event.status}, 围栏id: ${event.fenceId}, 自定义id: ${event.customId}');
    });
